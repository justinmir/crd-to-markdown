...
func readLinkFile(filePath string) (map[string]string, error) {
	links := map[string]string{}
...
func printTypesTOC(types []KubeTypes, testFunc func(string) bool) {
...
func printAPIDocs(conf *Config) error {
	var err error

	// print header
	printFile(conf.HeaderFilePath)

	links, err = readLinkFile(conf.ExternalLinksFile)
	if err != nil {
		return err
	}
